...
def generate_ip2p(argdict, outpath):
    ...
    load.load_ip2p(model)
    sampler = argdict.get("sampler")
    load.set_scheduler(sampler)
    ...
    image = load.pipe(prompt, negative_prompt = prompt_neg, image = image, num_inference_steps = steps, guidance_scale = cfg_txt, image_guidance_scale = cfg_img, generator = rng).images[0]
...
def generate_sd_onnx(argdict, outpath):
    ...
    eta = 0.0
    if mode == "txt2img":
        image = load.pipe(prompt = prompt, height = height, width = width, num_inference_steps = steps, guidance_scale = scale, negative_prompt = prompt_neg, generator = rng).images[0]
    if mode == "img2img":
        img = Image.open(init_img_path).convert('RGB')
        image = load.pipe(prompt = prompt, image = img, num_inference_steps = steps, guidance_scale = scale, negative_prompt = prompt_neg, eta = eta, strength = init_strength, generator = rng).images[0]
    if mode == "inpaint":
        img = Image.open(init_img_path).convert('RGB')
        mask = Image.open(mask_img_path)
        image = load.pipe(prompt = prompt, image = img, mask_image = mask, height = height, width = width, num_inference_steps = steps, guidance_scale = scale, negative_prompt = prompt_neg, eta = eta, generator = rng).images[0]
    if mode == "inpaint-legacy":
        img = Image.open(init_img_path).convert('RGB')
        mask = Image.open(mask_img_path)
        image = load.pipe(prompt = prompt, image = img, mask_image = mask, num_inference_steps = steps, guidance_scale = scale, negative_prompt = prompt_neg, eta = eta, strength = init_strength, generator = rng).images[0]
...
def generate_sd_xl(argdict, outpath, opt_load, seq_load):
    ...
    mode = argdict.get("mode")
    load.load_sdxl(model, refiner if not seq_load else "", mode, opt_load) # Load base+refiner (or only base, if seq_load is True)
    sampler = argdict.get("sampler")
    load.set_scheduler(sampler)
    prompt = argdict.get("prompt")
    prompt_neg = argdict.get("promptNeg")
    ...
    # Generate
    if mode == "txt2img":
        image = load.pipe(prompt = prompt, height = height, width = width, num_inference_steps = steps, guidance_scale = scale, negative_prompt = prompt_neg, generator = g, output_type = base_img_type, denoising_end = refine_frac).images[0]
    if mode == "img2img":
        img = Image.open(init_img_path).convert('RGB')
        image = load.pipe(prompt = prompt, image = img, strength = init_strength, num_inference_steps = steps, guidance_scale = scale, negative_prompt = prompt_neg, generator = g, output_type = base_img_type).images[0]
    if mode == "inpaint":
        img = Image.open(init_img_path).convert('RGB')
        mask = Image.open(mask_img_path)
        image = load.pipe(prompt = prompt, image = img, mask_image = mask, strength = init_strength, height = img.height, width = img.width, num_inference_steps = steps, guidance_scale = scale, negative_prompt = prompt_neg, generator = g, output_type = base_img_type).images[0]
    
    if seq_load:
        load.unload(True, False) # Unload base model
        load.load_sdxl("", refiner, mode, opt_load) # Load refiner
        load.set_scheduler(sampler)
    
    # Refine (optional)
    if do_refine:
        print(f'SDXL: Running refine model @ {refine_frac}')
        image = load.refiner(prompt = prompt, num_inference_steps = steps, denoising_start = refine_frac, guidance_scale = scale, negative_prompt = prompt_neg, generator = g, image = image[None, :]).images[0]
