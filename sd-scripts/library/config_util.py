...
class ConfigSanitizer:
  ...
  def __init__(self, support_dreambooth: bool, support_finetuning: bool, support_dropout: bool) -> None:
    assert support_dreambooth or support_finetuning, "Neither DreamBooth mode nor fine tuning mode specified. Please specify one mode or more. / DreamBooth ... fine tuning ...1..."
    ...
    if support_dreambooth and support_finetuning:
      def validate_flex_dataset(dataset_config: dict):
        ...
        # NOTE: all DB subsets should have no "metadata_file"
          ...
          raise voluptuous.Invalid("DreamBooth subset and fine tuning subset cannot be mixed in the same dataset. Please split them into separate datasets. / DreamBooth...fine tunin...")
...
def load_user_config(file: str) -> dict:
  ...
  if file.name.lower().endswith('.json'):
    try:
      ...
      print(f"Error on parsing JSON config file. Please check the format. / JSON ...: {file}")
      raise
  elif file.name.lower().endswith('.toml'):
    try:
      config = toml.load(file)
    except Exception:
      print(f"Error on parsing TOML config file. Please check the format. / TOML ...: {file}")
      raise
  else:
