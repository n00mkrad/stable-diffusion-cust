# ...train_dreambooth.py...Apache License 2.0...
...
def process(args):
  assert (not args.resize_fit) or args.resize_face_size is None, f"resize_fit and resize_face_size can't be specified both / resize_fit...resize_face_size..."
  assert args.crop_ratio is None or args.resize_face_size is None, f"crop_ratio...resize_face_size..."
  ...
  if args.crop_size is None:
    ...
    assert len(tokens) == 2, f"crop_size must be 'width,height' / crop_size...'...,...'..."
  ...
  if args.crop_ratio is None:
    ...
    assert len(tokens) == 2, f"crop_ratio must be 'horizontal,vertical' / crop_ratio...'...,...'..."
      ...
      if crop_width is not None or crop_h_ratio is not None:
        ...
        if args.resize_face_size is not None:
          ...
          if scale < cur_crop_width / w:
            print(
                f"image width too small in face size based resizing / ...crop size...: {path}")
            scale = cur_crop_width / w
          if scale < cur_crop_height / h:
            print(
                f"image height too small in face size based resizing / ...crop size...: {path}")
          ...
          # ...
          if w < cur_crop_width:
            print(f"image width too small/ ...crop size...: {path}")
            scale = cur_crop_width / w
          if h < cur_crop_height:
            print(f"image height too small/ ...crop size...: {path}")
            scale = cur_crop_height / h
          if args.resize_fit:
...
def setup_parser() -> argparse.ArgumentParser:
  ...
  parser.add_argument("--rotate", action="store_true", help="rotate images to align faces / ...")
  parser.add_argument("--resize_fit", action="store_true",
                      help="resize to fit smaller side after cropping / ...crop_size...")
  parser.add_argument("--resize_face_size", type=int, default=None,
                      help="resize image before cropping by face size / ...")
  parser.add_argument("--crop_size", type=str, default=None,
                      help="crop images with 'width,height' pixels, face centered / ...'...,...'...")
  parser.add_argument("--crop_ratio", type=str, default=None,
                      help="crop images with 'horizontal,vertical' ratio to face, face centered / ...'...,...'...")
  parser.add_argument("--min_size", type=int, default=None,
                      help="minimum face size to output (included) / ...")
  parser.add_argument("--max_size", type=int, default=None,
                      help="maximum face size to output (excluded) / ...")
  parser.add_argument("--multiple_faces", action="store_true",
                      help="output each faces / ...")
  parser.add_argument("--debug", action="store_true", help="render rect for face / ...")
