...
V2_STABLE_DIFFUSION_PATH = "stabilityai/stable-diffusion-2"  # ...tokenizer...
...
# region ...lpw_stable_diffusion.py ...ASL...
...
class PipelineLike:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if self.clip_guidance_scale > 0:
            ...
            text_embeddings_clip = text_embeddings_clip / text_embeddings_clip.norm(p=2, dim=-1, keepdim=True)  # prompt...OK
        ...
        if (
            ...
            if self.clip_image_guidance_scale > 0:
                ...
                size = (width // VGG16_INPUT_RESIZE_DIV, height // VGG16_INPUT_RESIZE_DIV)  # ...1/4...?...
    ...
    def cond_fn_vgg16_b1(self, latents, timestep, index, text_embeddings, noise_pred_original, guide_embeddings, guidance_scale):
        ...
        # ...
        loss = ((image_embeddings - guide_embeddings) ** 2).mean() * guidance_scale  # MSE style transfer...MSE...
        ...
        for i in range(max_embeddings_multiples):
            ...
            if pad == eos:  # v1
                ...
                for j in range(len(text_input_chunk)):
                    ...
                    if text_input_chunk[j, 1] == pad:  # BOS...PAD
                        text_input_chunk[j, 1] = eos
...
def preprocess_guide_image(image):
    image = image.resize(FEATURE_EXTRACTOR_SIZE, resample=Image.NEAREST)  # cond_fn...
...
def preprocess_vgg16_guide_image(image, size):
    image = image.resize(size, resample=Image.NEAREST)  # cond_fn...
    image = np.array(image).astype(np.float32) / 255.0
    image = image[None].transpose(0, 3, 1, 2)  # nchw
    image = torch.from_numpy(image)
    return image  # 0 to 1
...
def main(args):
    ...
    assert not highres_fix or args.image_path is None, f"highres_fix doesn't work with img2img / highres_fix...img2img..."
    ...
    elif args.sampler == "ddpm":  # ddpm...option...
    ...
    # Extended Textual Inversion ... Textual Inversion...
    ...
    if args.textual_inversion_embeddings:
        ...
        for embeds_file in args.textual_inversion_embeddings:
            ...
            if type(embeds) != torch.Tensor:
                raise ValueError(f"weight file does not contains Tensor / ...Tensor...: {embeds_file}")
            ...
            ), f"tokenizer has same word to token string (filename). please rename the file / ...: {embeds_file}"
    ...
    def resize_images(imgs, size):
        ...
        for img in imgs:
            ...
            if hasattr(img, "filename"):  # filename...
    ...
    if args.image_path is not None:
        ...
        assert len(init_images) > 0, f"No image / ...: {args.image_path}"
    ...
    if args.mask_path is not None:
        ...
        assert len(mask_images) > 0, f"No mask image / ...: {args.image_path}"
    ...
    if args.guide_image_path is not None:
        ...
        if len(guide_images) == 0:
            print(f"No guide image, use previous generated image. / ...: {args.image_path}")
            guide_images = None
    else:
        guide_images = None
        ...
        def process_batch(batch: List[BatchData], highres_fix, highres_1st=False):
            ...
            if highres_fix and not highres_1st:
                ...
                if upscaler:
                    ...
                    # ...PIL.Image.Image...torch.Tensor...latents
                    ...
                    if images_1st.dtype == torch.bfloat16:
                        images_1st = images_1st.to(torch.float)  # interpolate...bf16...
            ...
            for seed in seeds:  # images_per_prompt...
                ...
                if init_images is not None:
                    ...
                    if width != init_image.size[0] or height != init_image.size[1]:
                        print(
                            f"img2img image size is not divisible by 32 so aspect ratio is changed / img2img...32..."
                        )
...
def setup_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--interactive", action="store_true", help="interactive mode (generates one image) / ...1..."
    )
    parser.add_argument(
        "--no_preview", action="store_true", help="do not show generated image in interactive mode / ..."
    )
    parser.add_argument(
        "--image_path", type=str, default=None, help="image to inpaint or to generate from / img2img...inpaint..."
    )
    parser.add_argument("--mask_path", type=str, default=None, help="mask in inpainting / inpaint...")
    parser.add_argument("--strength", type=float, default=None, help="img2img strength / img2img...strength")
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="batch size for VAE, < 1.0 for ratio / VAE...1...",
    )
    ...
    parser.add_argument("--ckpt", type=str, default=None, help="path to checkpoint of model / ...checkpoint...")
    parser.add_argument(
    ...
    #                     help="Replace CLIP (Text Encoder) to l/14@336 / CLIP(Text Encoder)...l/14@336...")
    parser.add_argument(
        "--seed",
        type=int,
        default=None,
        help="seed, or seed of seeds in multiple generation / 1...seed...seed...seed",
    )
    parser.add_argument(
        "--iter_same_seed",
        action="store_true",
        help="use same seed for all prompts in iteration if no seed specified / ...seed...seed...",
    )
    parser.add_argument("--fp16", action="store_true", help="use fp16 / fp16...")
    parser.add_argument("--bf16", action="store_true", help="use bfloat16 / bfloat16...")
    parser.add_argument("--xformers", action="store_true", help="use xformers / xformers...")
    parser.add_argument(
        "--diffusers_xformers",
        action="store_true",
        help="use xformers by diffusers (Hypernetworks doesn't work) / Diffusers...xformers...Hypernetwork...",
    )
    parser.add_argument(
        "--opt_channels_last", action="store_true", help="set channels last option to model / ...channels last..."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=None,
        nargs="*",
        help="Embeddings files of Textual Inversion / Textual Inversion...embeddings",
    )
    parser.add_argument(
        "--XTI_embeddings",
        type=str,
        default=None,
        nargs="*",
        help="Embeddings files of Extended Textual Inversion / Extended Textual Inversion...embeddings",
    )
    parser.add_argument("--clip_skip", type=int, default=None, help="layer number from bottom to use in CLIP / CLIP...n...")
    parser.add_argument(
        "--max_embeddings_multiples",
        type=int,
        default=None,
        help="max embeding multiples, max token length is 75 * multiples / ... 75*... ...",
    )
    parser.add_argument(
        "--clip_guidance_scale",
        type=float,
        default=0.0,
        help="enable CLIP guided SD, scale for guidance (DDIM, PNDM, LMS samplers only) / CLIP guided SD...scale...DDIM...PNDM...LMS...",
    )
    parser.add_argument(
        "--clip_image_guidance_scale",
        type=float,
        default=0.0,
        help="enable CLIP guided SD by image, scale for guidance / ...CLIP guided SD...scale...",
    )
    parser.add_argument(
        "--vgg16_guidance_scale",
        type=float,
        default=0.0,
        help="enable VGG16 guided SD by image, scale for guidance / ...VGG16 guided SD...scale...",
    )
    parser.add_argument(
        "--vgg16_guidance_layer",
        type=int,
        default=20,
        help="layer of VGG16 to calculate contents guide (1~30, 20 for conv4_2) / VGG16...contents guide... (1~30...20...conv4_2)",
    )
    parser.add_argument(
        "--guide_image_path", type=str, default=None, nargs="*", help="image to CLIP guidance / CLIP guided SD..."
    )
    parser.add_argument(
        "--highres_fix_scale",
        type=float,
        default=None,
        help="enable highres fix, reso scale for 1st stage / highres fix...scale...",
    )
    parser.add_argument(
        "--highres_fix_steps", type=int, default=28, help="1st stage steps for highres fix / highres fix..."
    )
    parser.add_argument(
        "--highres_fix_save_1st", action="store_true", help="save 1st stage images for highres fix / highres fix..."
    )
    parser.add_argument(
        "--highres_fix_latents_upscaling",
        action="store_true",
        help="use latents upscaling for highres fix / highres fix...latent...",
    )
    parser.add_argument(
        "--highres_fix_upscaler", type=str, default=None, help="upscaler module for highres fix / highres fix...upscaler..."
    )
    parser.add_argument(
        "--highres_fix_upscaler_args",
        type=str,
        default=None,
        help="additional argmuments for upscaler (key=value) / upscaler...",
    )

    parser.add_argument(
        "--negative_scale", type=float, default=None, help="set another guidance scale for negative prompt / ...scale..."
    )

    parser.add_argument(
        "--control_net_models", type=str, default=None, nargs="*", help="ControlNet models to use / ...ControlNet..."
    )
    parser.add_argument(
        "--control_net_preps", type=str, default=None, nargs="*", help="ControlNet preprocess to use / ...ControlNet..."
    )
    parser.add_argument("--control_net_weights", type=float, default=None, nargs="*", help="ControlNet weights / ControlNet...")
    parser.add_argument(
        "--control_net_ratios",
        type=float,
        default=None,
        nargs="*",
        help="ControlNet guidance ratio for steps / ControlNet...",
    )
    # parser.add_argument(
    #     "--control_net_image_path", type=str, default=None, nargs="*", help="image for ControlNet guidance / ControlNet..."
    # )
