...
def interrogate(args):
  ...
  if not has_te_weight:
    print("This LoRA does not have modules for Text Encoder, cannot interrogate / ...LoRA...Text Encoder...")
    return
  del vae
  ...
  def get_all_embeddings(text_encoder):
    ...
    with torch.no_grad():
      for token_id in tqdm(range(token_id_start, token_id_end + 1, args.batch_size)):
        ...
        # batch_embs = text_encoder(torch.tensor(batch).to(DEVICE))[0].to("cpu")  # bos/eos... [:, 1]
        # clip skip...
  ...
  print("You can ignore warning messages start with '_IncompatibleKeys' (LoRA model does not have alpha because trained by older script) / '_IncompatibleKeys'...LoRA...alpha...")
  ...
  for i, (orig_emb, lora_emb) in enumerate(zip(orig_embs, tqdm(lora_embs))):
    ...
    # diff = torch.mean(torch.cosine_similarity(orig_emb, lora_emb, dim=1))       # ...
...
def setup_parser() -> argparse.ArgumentParser:
  ...
  parser.add_argument("--sd_model", type=str, default=None,
                      help="Stable Diffusion model to load: ckpt or safetensors file / ...SD...ckpt...safetensors")
  parser.add_argument("--model", type=str, default=None,
                      help="LoRA model to interrogate: ckpt or safetensors file / ...LoRA...ckpt...safetensors")
  parser.add_argument("--batch_size", type=int, default=16,
                      help="batch size for processing with Text Encoder / Text Encoder...")
