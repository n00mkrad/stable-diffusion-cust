...
def merge_lora_models(models, ratios, merge_dtype):
  ...
  for model, ratio in zip(models, ratios):
    ...
    for key in lora_sd.keys():
      if 'alpha' in key:
        if key in merged_sd:
          assert merged_sd[key] == lora_sd[key], f"alpha mismatch / alpha..."
        else:
...
def setup_parser() -> argparse.ArgumentParser:
  ...
  parser.add_argument("--precision", type=str, default="float",
                      choices=["float", "fp16", "bf16"], help="precision in merging (float is recommended) / ...float...")
  ...
  parser.add_argument("--models", type=str, nargs='*',
                      help="LoRA models to merge: ckpt or safetensors file / ...LoRA...ckpt...safetensors")
  parser.add_argument("--ratios", type=float, nargs='*',
                      help="ratios for each model / ...LoRA...")
