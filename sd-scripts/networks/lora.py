...
# block_dims, block_alphas ...None...
# conv_dim, conv_alpha ...None...
def get_block_dims_and_alphas(
    ...
    if block_dims is not None:
        ...
        ), f"block_dims must have {num_total_blocks} elements / block_dims...{num_total_blocks}..."
    else:
        print(f"block_dims is not specified. all dims are set to {network_dim} / block_dims...dim...{network_dim}...")
    ...
    if block_alphas is not None:
        ...
        ), f"block_alphas must have {num_total_blocks} elements / block_alphas...{num_total_blocks}..."
    else:
        print(
            f"block_alphas is not specified. all alphas are set to {network_alpha} / block_alphas...alpha...{network_alpha}..."
        )
        block_alphas = [network_alpha] * num_total_blocks

    # conv_block_dims...conv_block_alphas...conv_dim...conv_alpha...
    if conv_block_dims is not None:
        conv_block_dims = parse_ints(conv_block_dims)
        assert (
            len(conv_block_dims) == num_total_blocks
        ), f"conv_block_dims must have {num_total_blocks} elements / conv_block_dims...{num_total_blocks}..."

        if conv_block_alphas is not None:
            conv_block_alphas = parse_floats(conv_block_alphas)
            assert (
                len(conv_block_alphas) == num_total_blocks
            ), f"conv_block_alphas must have {num_total_blocks} elements / conv_block_alphas...{num_total_blocks}..."
        else:
            if conv_alpha is None:
                conv_alpha = 1.0
            print(
                f"conv_block_alphas is not specified. all alphas are set to {conv_alpha} / conv_block_alphas...alpha...{conv_alpha}..."
            )
            conv_block_alphas = [conv_alpha] * num_total_blocks
    else:
        if conv_dim is not None:
            print(
                f"conv_dim/alpha for all blocks are set to {conv_dim} and {conv_alpha} / ...conv_dim...alpha...{conv_dim}...{conv_alpha}..."
            )
...
def get_block_lr_weight(
    ...
    max_len = LoRANetwork.NUM_OF_BLOCKS  # ...up,down...
    ...
    def get_list(name_with_suffix) -> List[float]:
        ...
        if name == "cosine":
            ...
            print(
                "Unknown lr_weight argument %s is used. Valid arguments:  / ...lr_weight... %s ...:\n\tcosine, sine, linear, reverse_linear, zeros"
                % (name)
            )
            return None
    ...
    if (up_lr_weight != None) or (mid_lr_weight != None) or (down_lr_weight != None):
        ...
        if down_lr_weight != None:
            ...
            print("down_lr_weight: all 1.0, ...1.0")
        ...
        if up_lr_weight != None:
            ...
            print("up_lr_weight: all 1.0, ...1.0")

    return down_lr_weight, mid_lr_weight, up_lr_weight


# lr_weight...0...block...block_dims...
...
class LoRANetwork(torch.nn.Module):
    ...
    def __init__(
        ...
        LoRA network: ...
        1. lora_dim...alpha...
        2. lora_dim...alpha...conv_lora_dim...conv_alpha...
        3. block_dims...block_alphas... :  Conv2d3x3...
        4. block_dims...block_alphas...conv_block_dims...conv_block_alphas... : Conv2d3x3...
        5. modules_dim...modules_alpha... (...)
        """
        super().__init__()
        self.multiplier = multiplier
        ...
        if varbose and len(skipped) > 0:
            print(
                f"because block_lr_weight is 0 or dim (rank) is 0, {len(skipped)} LoRA modules are skipped / block_lr_weight...dim (rank)...0...{len(skipped)}...LoRA...:"
            )
            for name in skipped:
                print(f"\t{name}")
